...
class DSTMultiWozData:
    def __init__(self, model_name, tokenizer, data_path_prefix, shuffle_mode='shuffle_session_level', 
        ...
        if self.add_prefix:
            bs_prefix_text = 'translate dialogue to dialogue action:'
            self.da_prefix_id = self.tokenizer.convert_tokens_to_ids(tokenizer.tokenize(bs_prefix_text))
        else:
            self.da_prefix_id = []
    ...
    def tokenize_raw_data(self, raw_data_list):
        ...
        for idx in range(data_num):
            ...
            for turn in raw_data_list[idx]:
                one_turn_dict = {}
                for key in turn:
                    if key in ['dial_id', 'pointer', 'turn_domain', 'turn_num','bspn', 'bsdx', 'bspn_reform', 'bsdx_reform']:
                        one_turn_dict[key] = turn[key]
                    else:
                        # only tokenize ["user", "usdx", "resp", 'aspn', 'dspn', 'aspn_reform', 'db']
                        value_text = turn[key]
    ...
    def parse_one_eva_instance(self, one_instance,ref_db, input_contain_db):
        '''
        ...
        if input_contain_db:
            ...
            da_input_id_list = self.da_prefix_id + [self.sos_context_token_id] + \
            da_input_id_list[-900:] + [self.eos_context_token_id]
        return da_input_id_list, res_dict
    ...
    def build_all_evaluation_batch_list(self, ref_db, input_contain_db, eva_batch_size, eva_mode):
        '''
        ...
        all_da_input_id_list, all_parse_dict_list = [], []
        for item in data_list:
            one_da_input_id_list, one_parse_dict = self.parse_one_eva_instance(item, ref_db, input_contain_db)
            all_da_input_id_list.append(one_da_input_id_list)
            all_parse_dict_list.append(one_parse_dict)
        assert len(all_da_input_id_list) == len(all_parse_dict_list)
        ...
        batch_num = len(da_batch_list)
        final_batch_list = []
        for idx in range(batch_num):
            one_final_batch = [da_batch_list[idx], parse_dict_batch_list[idx]]
            if len(da_batch_list[idx]) == 0: 
                continue
            else:
